...
scene("win", ({ score }) => {
    ...
    add([text("YOU WIN! You get 100 coins."), { size: 48 }, pos(width() / 2, height() / 2 - 250), scale(2), anchor("center")]);
    add([sprite("player"), { size: 30 }, pos(width() / 2, height() / 2 - 128), scale(0.3), anchor("center")]);
    add([text(`Coins: ${coins}`), { size: 30 }, pos(width() / 2, height() / 2 + 200), scale(2), anchor("center")]);
    add([text(`Score: ${score} seconds`), { size: 30 }, pos(width() / 2, height() / 2), scale(2), anchor("center")]);
    add([text(`Highscore: ${highscores} seconds`), { size: 30 }, pos(width() / 2, height() / 2 + 100), scale(2), anchor("center")]);
...
scene("lose", () => {
    add([text("YOU LOSE!"), { size: 48 }, pos(width() / 2, height() / 2 - 250), scale(2), anchor("center")]);
    add([sprite("player"), { size: 30 }, pos(width() / 2, height() / 2 - 128), scale(0.3), anchor("center")]);
    add([text(`Highscore: ${highscores} seconds`), { size: 30 }, pos(width() / 2, height() / 2), scale(2), anchor("center")]);
    add([text(`Total Coins: ${coins}`), { size: 30 }, pos(width() / 2, height() / 2 + 100), scale(2), anchor("center")]);
