...
scene("win", ({ score }) => {
    ...
    add([sprite("player", { size: 20 }), pos(width() / 2, height() / 2 - 128), scale(0.3), anchor("center")]);
    add([text(`Coins: ${coins}`, { size: 20 }), pos(width() / 2, height() / 2 + 200), scale(2), anchor("center")]);
    add([text(`Score: ${score} seconds`, { size: 20 }), pos(width() / 2, height() / 2), scale(2), anchor("center")]);
    add([text(`Highscore: ${highscores} seconds`, { size: 20 }), pos(width() / 2, height() / 2 + 100), scale(2), anchor("center")]);
